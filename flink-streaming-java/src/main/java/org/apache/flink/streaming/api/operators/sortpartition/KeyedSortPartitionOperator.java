 ...
 * record will be written to {@link ExternalSorter} directly in most cases. However, if the record
 * is sorted according to the selected key by {@link KeySelector}, the selected sort key should also
 * be written with the record key and the record to {@link ExternalSorter} to avoid repeated key
 * selections.
 *
